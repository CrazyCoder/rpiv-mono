...
- `ask_user_question` now honours the agent's `AbortSignal`. The tool declared its signal parameter unused and waited on `ctx.ui.custom()`, which settles only when the overlay calls `done()`. If a turn is aborted while the questionnaire is open and the Esc keystroke reaches the editor rather than the overlay — a collapsed overlay, or one bricked by a sibling `ui.custom()` ([pi #5129](https://github.com/earendil-works/pi/issues/5129)) — nothing can settle that promise, and the agent loop, which awaits the tool, can never finalise the turn. This is a latent defect found by inspection and reproduced in tests, not one diagnosed from a field report: without the fix three of the new regression cases hang until the runner times them out. Answering normally with Enter was never affected. The wait is now raced against the signal on both the TUI and RPC paths, and the questionnaire is dismissed through `done()`, falling back to the overlay handle when the abort beats the factory.
## [2.8.0] - 2026-08-29
