...
| `feat(ask-user-question): yield the tool name to an installed mirror` | fork-only | The extension registers nothing when the user's own settings install a mirror package that wraps this questionnaire. Pi makes two extensions registering one tool name a fatal load error, and resolves packages with project scope beating user scope, so a user running a mirror cannot stop a shared project `.pi/settings.json` from loading this extension alongside it. Only this package can break that tie by declining the name. Detection reads user-scope settings only and fails closed, so a user without a mirror is unaffected. Not PR material: upstream has no mirror to defer to. |
| `fix(ask-user-question): declare typebox as a peer dependency` | none yet | `typebox` moves from `dependencies` to `peerDependencies` with a `"*"` range. Pi supplies it to extensions, and since the fix for pi #9863 it warns at every start about an extension listing a host-provided module in `dependencies`. Upstream has the same manifest in this and its other extension packages, so this is PR material once it covers all of them. |
